...
```bash
uv run python -m playground.scripts.train
```
...
uv run python -m playground.scripts.train model.optimizer.lr=0.001 data.batch_size=64

# Train for specific number of epochs
uv run python -m playground.scripts.train trainer.max_epochs=50
...
uv run python -m playground.scripts.train trainer.devices=2

# Multi-GPU training with specific strategy
uv run python -m playground.scripts.train trainer.devices=4 trainer.strategy=ddp
```
...
```bash
uv run python -m playground.scripts.train trainer.accelerator=cpu
```
...
```bash
uv run python -m playground.scripts.train trainer.precision=16-mixed
```
...
```bash
uv run python -m playground.scripts.train ckpt_path="/path/to/checkpoint.ckpt"
```
...
uv run python -m playground.scripts.train experiment=mnist_experiment

# Override experiment parameters
uv run python -m playground.scripts.train experiment=mnist_experiment model.optimizer.lr=0.01
```
...
```bash
uv run python -m playground.scripts.train logger=many_loggers
```
...
```bash
# Standard single GPU
uv run python -m playground.scripts.train trainer.devices=1
...
# Node 0 (master)
uv run python -m playground.scripts.train trainer.devices=4 trainer.num_nodes=$NUM_NODES
...
uv run python -m playground.scripts.train callbacks.model_checkpoint.save_top_k=3 \
...
```bash
uv run python -m playground.scripts.train callbacks.model_checkpoint.every_n_epochs=10
```
...
uv run python -m playground.scripts.train callback.progress_bar=rich
```
...
```bash
uv run python -m playground.scripts.train callbacks.early_stopping.monitor="val/loss" \
...
```bash
# limit training data
uv run python -m playground.scripts.train debug=limit
```
...
uv run python -m playground.scripts.train -m model.optimizer.lr=0.001,0.01,0.1

# Random search with Optuna
uv run python -m playground.scripts.train -m hparams_search=optuna experiment=example
```
...
```bash
uv run python -m playground.scripts.train debug=profiler trainer.profiler=simple

# Advanced profiling
uv run python -m playground.scripts.train debug=profiler trainer.profiler=advanced
```
