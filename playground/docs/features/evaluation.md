...
```bash
uv run python -m playground.scripts.eval
```
...
```bash
uv run python -m playground.scripts.eval ckpt_path="/path/to/checkpoint.ckpt"
```
...
```bash
uv run python -m playground.scripts.eval data.test_path="/path/to/test/data"
```
...
```bash
uv run python -m playground.scripts.eval trainer.accelerator=cpu
```
...
# Specify GPU device
uv run python -m playground.scripts.eval trainer.devices=1 trainer.accelerator=gpu
```
...
```bash
uv run python -m playground.scripts.eval trainer.devices=4 trainer.strategy=ddp
```
...
uv run python -m playground.scripts.eval data.batch_size=128

# Smaller batch if memory constrained
uv run python -m playground.scripts.eval data.batch_size=32
```
...
```bash
uv run python -m playground.scripts.eval trainer.logger.level=DEBUG
```
...
```bash
uv run python -m playground.scripts.eval debug=limit
```
...
```bash
uv run python -m playground.scripts.eval debug=profiler
```
...
```bash
# Reduce batch size
uv run python -m playground.scripts.eval data.batch_size=16

# Use CPU evaluation
uv run python -m playground.scripts.eval trainer.accelerator=cpu

# Use lower precision
uv run python -m playground.scripts.eval ++trainer.precision=16-mixed
```
...
uv run python -m playground.scripts.eval ckpt_path="/absolute/path/to/checkpoint.ckpt"
...
uv run python -m playground.scripts.eval data.num_workers=8

# Enable pin memory
uv run python -m playground.scripts.eval data.pin_memory=true

# Use compiled model
uv run python -m playground.scripts.eval model.torch_compile=true
```
